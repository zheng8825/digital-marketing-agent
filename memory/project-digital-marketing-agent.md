...
**FIXED 2026-05-12 — THE "agent never answers / Thinking… forever" bug.** Root cause: `server.ts`
used `req.on('close', () => handle.cancel())` on the streaming `/api/chat` (and `/api/setup/run`)
endpoints. On Node 24 (the user's), the request's `'close'` fires as soon as `express.json()` has
consumed the POST body — i.e. ~immediately — so every chat turn's `claude` process was killed right
after spawning, and since `cancel()` calls `finish()` with no event, the renderer never got a
`done`/`error` → stuck on "Thinking…". (Same cause: the SwitchAccountModal's `claude auth logout` /
`claude login` steps showed `(cancelled)` instantly.) **Fix:** listen on `res.on('close')` (the
*response*'s close — fires only on a real mid-stream disconnect or after `res.end()`), guarded by a
`finished` flag so a normal completion doesn't trigger `cancel()`. **Verified** by running
`electron-vite preview`, finding the API port, and `POST /api/chat` → got delta + `done` in ~4s (and
note: `costUsd ≈ 0.04` with small token counts → confirms the agent is on Sonnet now). This bug had
likely been latent since the first scaffold (the chat was apparently never exercised through the GUI).
Also: `openTerminal(runCommand?)` can now pre-run a command; `POST /api/auth/switch-terminal` opens a
terminal running `claude auth logout; claude login` — the SwitchAccountModal now leads with that (the
in-app streamed login is best-effort/fragile for an interactive OAuth flow).

**Added 2026-05-12 (proper Windows .ico):** the portable `.exe` was showing a blank/partial icon —
electron-builder's PNG→ICO auto-conversion of `build/icon.png` was unreliable. Fix: commit a real
multi-resolution `build/icon.ico` (256/48/32/16, 32bpp) generated by `node build/make-icon.mjs`
(uses the `png-to-ico` devDep; `npm run icon` is the shortcut). `electron-builder.yml`: `win.icon` →
`build/icon.ico`, plus `nsis.installerIcon`/`uninstallerIcon`/`installerHeaderIcon`. `build/icon.png`
stays the source artwork + the dev-window icon. To change the icon: replace `icon.png` (keep square),
`npm run icon`, rebuild. Rebuilt + verified: both exes now embed the custom robot icon (sampled the
extracted icon — blue gradient, not the default Electron teal). (Source PNG has white — not transparent —
corners; if that ever bugs anyone, supply a transparent-corner PNG or add a rounded-rect alpha mask in
`make-icon.mjs`.)
